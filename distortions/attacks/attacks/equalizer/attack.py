...
class EqualizerAttack(BaseAttack):
    ...
    def apply(self, audio: np.ndarray, **kwargs) -> np.ndarray:
        ...
        center_freqs = [
            ...
            8000,
            16000,
        ]  # add 8000, 16000 for higher sample rates
        ...
        self.filters.append(
            self.Biquad(
                sampling_rate, "LowShelf", center_freqs[0], band_widths[0], gains[0]
            )
        )
        for i in range(1, 8 - 1):
            self.filters.append(
                self.Biquad(
                    sampling_rate, "Peaking", center_freqs[i], band_widths[i], gains[i]
                )
            )
