...
# Load config
with open("config/finetune_train_part.yaml", "r") as f:
...
pattern = re.compile(r"wm_model_pesq_([\d\.]+)_acc_([\d\.]+)\.pt")
...
for epoch in range(0, train_config["iter"]["epoch"] + 1):
    ...
    if save_model(
        ...
        checkpoint_path = os.path.join(
            ckpt_dir, f"finetune_wm_model_pesq_{curr_pesq}_acc_{curr_acc}.pt"
        )
        torch.save(
            {
                "epoch": epoch,
