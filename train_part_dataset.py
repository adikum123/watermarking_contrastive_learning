"""
Train audio watermarking model with contrastive learning on part of the dataset
"""
...
for epoch in range(start_epoch, train_config["iter"]["epoch"] + 1):
    ...
    if save_model(
        best_pesq=best_pesq, best_acc=best_acc, new_pesq=curr_pesq, new_acc=curr_acc
    ):
        best_acc, best_pesq = curr_acc, curr_pesq
        checkpoint_path = os.path.join(checkpoint_dir, "wm_model.pt")
        torch.save(
            {
                "epoch": epoch,
                "batch_idx": i,
